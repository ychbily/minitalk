...
/*   Updated: 2024/04/07 01:06:42 by ychbily          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minitalk.h"

void	ft_reset(t_data *data, siginfo_t *info)
{
	(void)info;
	data->bit = 0;
	data->byte = 0;
	data->uni_bit_counter = 0;
	data->uni_bytes= 4;
	while (data->uni_bytes > 0)
		data->unicode[--data->uni_bytes] = 0;
}

void	ft_unicode(t_data *data)
{
	if (data->uni_bytes == 0)
		data->uni_bytes = (data->byte >= 240) + \
		(data->byte >= 224) + (data->byte >= 192) * 2;
	if (data->uni_bytes && data->uni_bit_counter < data->uni_bytes)
		data->unicode[data->uni_bit_counter++] = data->byte;
	if (data->uni_bytes && data->uni_bit_counter == data->uni_bytes)
	{
